...
class Crontab extends CrontabServer
{
    ...
    public function onReceive(\swoole_server $server, int $fd, int $reactor_id, string $data)
    {
        ...
        //根据指令接口投递异步任务
        if (($data = json_decode($data, true)) && is_array($data) && isset($data['cmd'])) {
            //管理客户端指令接口
            if (($client_info = $server->getClientInfo($fd)) && (Config::loadConfig('crontab')->get('server_connect.adminPort') == $client_info['server_port'])) {
                switch (strtolower($data['cmd'])) {
                    case 'add':
                        Task::task($server, [$this->logic, 'add'], [$data['data']]);
                        break;
                    case 'start':
                        Task::task($server, [$this->logic, 'start'], [$data['data']]);
                        break;
                    case 'stop':
                        Task::task($server, [$this->logic, 'stop'], [$data['data']]);
                        break;
                    case 'delete':
                        Task::task($server, [$this->logic, 'delete'], [$data['data']]);
                        break;
                }
            }
        }
