...
use vSwoole\core\client\HttpClient;
...
class Http extends HttpClient
{
    ...
    public function __construct(array $connectOptions = [], array $configOptions = [])
    {
        if (empty($connectOptions)) {
            ...
            $serverKey = Config::loadConfig('redis')->get('redis_key.Http.Server_Ip');
    ...
    {
        $redis = Redis::getInstance(Config::loadConfig('redis')->get('redis_master'), true);
        $server_ips = $redis->SMEMBERS(Config::loadConfig('redis')->get('redis_key.Http.Server_Ip'));
    ...
    {
        $redis = Redis::getInstance(Config::loadConfig('redis')->get('redis_master'), true);
        $server_key = Config::loadConfig('redis')->get('redis_key.Http.Server_Ip');
